...
int sa_transform_matches(peer_ctx* ctx, struct isakmp_payload *t)
{
	...
	if(!(enc && hash && auth_method && group_desc)) {
		fprintf(stderr,
			"[%s:%d]: missing attribute(s): enc=%p hash=%p auth_method=%p group_desc=%p\n",
...
void sa_populate_from(peer_ctx* ctx, struct isakmp_payload *response, struct isakmp_payload *proposal)
{
	...
	*response->u.sa.proposals = *proposal->u.sa.proposals;
	...
	if(!p) {
		fprintf(stderr,
			"[%s:%d]: no matching algo proposal, ignoring request\n",
			inet_ntoa(ctx->peer_addr.sin_addr),
			ntohs(ctx->peer_addr.sin_port));
		return;
	}
 ...
 */
void ike_process_aggressive_new(int s, peer_ctx *ctx, struct isakmp_packet *ikp)
{
	/* get the payloads */
	...
	switch(p->type) {
		...
		case ISAKMP_PAYLOAD_VID:
			/*fprintf(stderr, "ISAKMP_PAYLOAD_VID (ignored)\n");*/
			/* ignore for now */
			break;
	...
	/* payload: ke */
	p->next = NULL /*new_isakmp_payload()*/;
...
void ike_process_aggressive(int s, peer_ctx *ctx, struct isakmp_packet *ikp)
{
	...
	switch(ctx->state) {
		...
		/* XXX */
		default:
			fprintf(stderr, "FATAL: illegal state in internal FSM\n");
			exit(-1);
			break;
	}
}
...
void ike_process_isakmp(int s, peer_ctx *ctx, struct isakmp_packet *ikp)
{
	...
	switch(ikp->exchange_type) {
		...
		default:
			fprintf(stderr,
				"[%s:%d]: unhandled exchange type 0x%02x\n",
				inet_ntoa(ctx->peer_addr.sin_addr),
				ntohs(ctx->peer_addr.sin_port),
				ikp->exchange_type);
			break;
